...
Set-VMReplicationServer $true -AllowedAuthenticationType Kerberos -DefaultStorageLocation D:\ -ReplicationAllowedFromAnyServer $true

# Setup Networking. Requires a network adapter "Trunk" in its name.
New-VMSwitch "Trunk" -EnableIov $true -NetAdapterName (Get-NetAdapter | Where-Object {$_.Name -like "*trunk*"}).Name -AllowManagementOS $true
